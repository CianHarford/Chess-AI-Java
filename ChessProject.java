...
private Stack getWhitePawnSquares(int x, int y, String piece){
  ...
  for(int i = 1; i <=2; i++) {
      ...
      for(int i = 1; i < 2; i++) {
          int tmpy = y + i;
          int tmpx = x + i;
          //to ensure the piece can't off the board
...
private void getStack(Stack stack, Stack completeMoves){
  ...
  while(!stack.empty()){
    ...
    /*
    possible moves that AI can makes
    *//*
    if(tmpString.contains(PieceValues.knight)){
      tmpMoves = getKnightMoves(s.getXC(), s.getYC(), s.getName());
    }
    else if(tmpString.contains(PieceValues.bishop)){
      tmpMoves = getBishopMoves(s.getXC(), s.getYC(), s.getName());
    }
    else if(tmpString.contains(PieceValues.pawn)){
      tmpMoves = getWhitePawnSquares(s.getXC(), s.getYC(), s.getName());
    }
    else if(tmpString.contains(PieceValues.rook)){
      tmpMoves = getRookMoves(s.getXC(), s.getYC(), s.getName());
    }
    else if(tmpString.contains(PieceValues.queen)){
        tmpMoves = getQueenMoves(s.getXC(), s.getYC(), s.getName());
    }
    else if(tmpString.contains(PieceValues.king)){
        tmpMoves = getKingSquares(s.getXC(), s.getYC(), s.getName());
    }
